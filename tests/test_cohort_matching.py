...
def test_exact_and_not_found():
    ...
    assert cohort["match_proportion"] == 0.5
...
def test_recent_cohort_and_absent_course_followup():
    ...
    assert all(
        c["followup_semesters"] is None
        for c in summarize_cohorts(only_entrants, match_cohorts(only_entrants))
    )
