...
def test_ingressantes_and_repetitions():
    ...
    r = result.records[0]
    assert (r.periodo, r.curso_original, r.turno) == ("2019/1", "ENGENHARIA DE SOFTWARE", "NOTURNO")
...
def test_graduates():
    ...
    assert (r.periodo, r.tipo_registro, r.numero_sequencial) == ("2025/2", "concluinte", 1)


def test_homonyms_kept():
    result = parse_ingressantes([ENTRANTS.replace("JOAO EXEMPLO SOUZA", "ANA TESTE SILVA")])
    assert result.registros_unicos == 2
...
def test_total_mismatch():
    assert parse_ingressantes([ENTRANTS.replace("Total: 2", "Total: 3")]).status == "WARNING"
...
def test_period_order_and_invalid():
    assert sorted(["2020/1", "2019/2", "2019/1"], key=period_index) == ["2019/1", "2019/2", "2020/1"]
