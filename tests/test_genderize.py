...
def response(names, gender="female", p=.95):
    return [{"name": n, "gender": gender, "probability": p, "count": 100} for n in names]


def test_persistent_cache_and_query_modes(tmp_path):
    path = tmp_path / "cache.sqlite"
    session = Mock()
    session.get.return_value = Mock(status_code=200, json=lambda: response(["ANA TESTE SILVA"]))
    ...
    with GenderCache(path) as cache:
        ...
        assert client.lookup(["ANA TESTE SILVA"])["ANA TESTE SILVA"]["probability"] == .95
        assert session.get.call_count == 1
        assert GenderizeClient(cache, mode="first").lookup(["ANA TESTE SILVA"], no_api=True)["ANA TESTE SILVA"] is None
        assert GenderizeClient(cache, country="PT").lookup(["ANA TESTE SILVA"], no_api=True)["ANA TESTE SILVA"] is None
...
def test_permanent_errors_do_not_retry(tmp_path, status):
    ...
    with GenderCache(tmp_path / "cache.sqlite") as cache:
        with pytest.raises(GenderizeError) as error:
            GenderizeClient(cache, "synthetic-test-value", session=session).lookup(["ANA TESTE SILVA"])
        assert "synthetic-test-value" not in str(error.value)
        assert session.get.call_count == 1


@pytest.mark.parametrize("failure", [429, 500, 502, 503, requests.Timeout("secret"), requests.ConnectionError("secret"), ValueError("secret")])
def test_recoverable_errors(tmp_path, failure):
    ...
    with GenderCache(tmp_path / "cache.sqlite") as cache:
        result = GenderizeClient(cache, "synthetic-test-value", session=session, sleep=sleep).lookup(["ANA TESTE SILVA"])
...
def test_exhaustion_and_no_api(tmp_path):
    ...
    with GenderCache(tmp_path / "cache.sqlite") as cache:
        client = GenderizeClient(cache, "synthetic-test-value", session=session, sleep=sleep)
        assert client.lookup(["ANA TESTE SILVA"], no_api=True) == {"ANA TESTE SILVA": None}
...
@pytest.mark.parametrize("gender,p,q,confidence", [
    ("female", .95, .95, "alta confiança"), ("male", .8, .2, "confiança moderada"),
    ("female", .6, .6, "baixa confiança"), (None, None, None, "não identificado"),
])
...
def test_malformed_probability_rejected():
    ...
    with pytest.raises(ValueError):
        apply_prediction(record, {"gender": "female", "probability": float("nan"), "count": 1})
