...
def validate_prediction(item: dict):
    if not isinstance(item, dict) or not {"gender", "probability", "count"} <= item.keys():
...
class GenderizeClient:
    def __init__(self, cache, api_key=None, country="BR", mode="full", batch_size=100,
                 session=None, sleep=time.sleep, attempts=4):
    ...
    def _batch(self, names: list[str]) -> list[dict]:
        if not self.api_key:
            raise GenderizeError("GENDERIZE_API_KEY ausente; configure .env ou use --no-api.")
        for attempt in range(self.attempts):
            try:
                response = self.session.get(
                    "https://api.genderize.io",
                    params={"name[]": names, "country_id": self.country, "apikey": self.api_key},
                    timeout=(10, 30), allow_redirects=False,
                )
                status = response.status_code
                if status in (401, 402):
                    raise GenderizeError(f"Genderize HTTP {status}: verifique credencial/assinatura.")
                if status in (429, 500, 502, 503):
                    raise requests.ConnectionError("Falha recuperável")
                if status != 200:
                    raise GenderizeError(f"Genderize HTTP {status}; consulta interrompida.")
                ...
                for name, item in zip(names, payload):
                    ...
                    if item.get("name") != name or item.get("country_id", self.country) != self.country:
                        raise ValueError("Resposta não corresponde à consulta")
                return payload
            except (requests.RequestException, ValueError, TypeError):
                if attempt + 1 == self.attempts:
                    raise GenderizeError("Genderize indisponível ou resposta inválida após tentativas limitadas.") from None
    ...
    def lookup(self, names: list[str], no_api=False) -> dict[str, dict | None]:
        ...
        results = {q: self.cache.get(q, self.country, self.mode) for q in set(queries.values())}
        ...
        if not no_api:
            for start in range(0, len(missing), self.batch_size):
                ...
                for query, item in zip(batch, self._batch(batch)):
                    result = dict(item, country_id=self.country, name_mode=self.mode,
...
def apply_prediction(record, prediction, high=0.90, medium=0.75):
    ...
    record.genderize_gender = record.genderize_probability = record.genderize_count = None
    ...
    record.confidence_class = "alta confiança" if p >= high else (
        "confiança moderada" if p >= medium else "baixa confiança")
