...
def match_cohorts(records: list) -> list[dict]:
    entrants = [
        (i, r) for i, r in enumerate(records) if r.tipo_registro == "ingressante"
    ]
    graduates = [
        (i, r) for i, r in enumerate(records) if r.tipo_registro == "concluinte"
    ]
        ...
        matches.append(
            {
                ...
                "curso_normalizado": e.curso_normalizado,
                "periodo_ingresso": e.periodo,
                "periodo_conclusao": records[j].periodo if exact else None,
                "semesters_to_graduation": semester_difference(
                    e.periodo, records[j].periodo
                )
                if exact
                else None,
            }
...
def summarize_cohorts(
    records: list, matches: list[dict], min_followup=None
) -> list[dict]:
    ...
    for g in grads:
        ...
        if course not in latest_course or period_index(g.periodo) > period_index(
    ...
    for (course, period), group in sorted(groups.items()):
        ...
        matched = [
            records[m["entrant_id"]] for m in group if m["status"] == "exact_unique"
        ]
        last_course = latest_course.get(course)
        followup = (
            max(0, semester_difference(period, last_course)) if last_course else None
        )
        if min_followup is not None and (followup is None or followup < min_followup):
            continue
        rows.append(
            {
                "curso_normalizado": course,
                "periodo_ingresso": period,
                "n_ingressantes": len(entrants),
                "resolved_entrants": sum(r.p_female is not None for r in entrants),
                "expected_female_entrants": sum(
                    r.p_female for r in entrants if r.p_female is not None
                ),
                "expected_male_entrants": sum(
                    r.p_male for r in entrants if r.p_male is not None
                ),
                "n_matches_exact_unique": len(matched),
                "resolved_matched": sum(r.p_female is not None for r in matched),
                "expected_female_matched": sum(
                    r.p_female for r in matched if r.p_female is not None
                ),
                "expected_male_matched": sum(
                    r.p_male for r in matched if r.p_male is not None
                ),
                "latest_graduation_period": latest,
                "latest_graduation_period_course": last_course,
