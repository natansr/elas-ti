...
def summarize(records: list) -> dict:
    ...
    lower, upper = quantile(pmf, 0.025), quantile(pmf, 0.975)
    probabilities = [r.genderize_probability for r in records if r.p_female is not None]
    counts = [
        r.genderize_count
        for r in records
        if r.p_female is not None and r.genderize_count is not None
    ]
    probabilities = [p for p in probabilities if p is not None]
    return {
        "total": n,
        "resolvidos": resolved,
        "nao_identificados": unknown,
        "cobertura": resolved / n if n else None,
        "expected_female": expected,
        "expected_male": resolved - expected,
        "female_percent_resolved": 100 * expected / resolved if resolved else None,
        "male_percent_resolved": 100 * (resolved - expected) / resolved
        if resolved
        else None,
        "variance_female": variance,
        "sd_female": math.sqrt(variance),
        "interval95_lower_resolved": lower if resolved else None,
        "interval95_upper_resolved": upper if resolved else None,
        "conservative_count_lower": lower,
        "conservative_count_upper": upper + unknown,
        ...
        "expected_female_percent_total_max": 100 * (expected + unknown) / n
        if n
        else None,
...
def temporal_summary(records: list, by_course=False) -> list[dict]:
    ...
    rows.sort(
        key=lambda row: (row.get("curso_normalizado", ""), period_index(row["periodo"]))
    )
    previous = {}
    for row in rows:
...
def compare_periods(entrants: list[dict], graduates: list[dict]) -> list[dict]:
    ...
    for period in sorted(left.keys() | right.keys(), key=period_index):
        ...
        rows.append(
            {
                "periodo": period,
                "female_percent_entrants": a,
                "female_percent_graduates": b,
                "difference_pp_graduates_minus_entrants": b - a
...
def monte_carlo_difference(
    q_a: list[float], q_b: list[float], iterations=100000, seed=42
) -> dict:
    ...
    values = np.quantile(difference, [0.025, 0.5, 0.975])
